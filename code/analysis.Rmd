...
returns.crash <- data[between(date, date.pre, date.trough)] %>% 
    ...
    group_by(ticker) %>% 
    summarize( ret.crash =  ( prod( 1 + exc.ret/100 )^(1/as.integer(date.trough-date.pre)) -1 ) *100   )
...
returns.recovery <- data[between(date, date.trough, date.post)] %>% 
    select(ticker, exc.ret) %>% 
    group_by(ticker) %>% 
    summarize( ret.recovery =  ( prod( 1 + exc.ret/100 )^(1/as.integer(date.post-date.trough)) -1 ) *100   )
...
sumvars <- c('beta' ,  'bv', 'BM', 'log.mkt.cap', 'assets','cash.st', 'cash.assets' , 'com.shares.out', 'debt.current', 'debt.lt', 'leverage', 'profitability' , 'ENSCORE', 'TRESGS', 'TRESGCS', 'users_holding', 'frac.users', 'ret.mom', 'log.tv', 'ret.crash', 'ret.recovery')
...
if (summarytools){
    stats = c('n.valid','min','q1', 'med', 'q3', 'max', 'sd') #for summarytools
} else{
    stats = c('NUnique','Min','P25', 'Median', 'P75', 'Max', 'SD')  #for modelsummary
}
...
for (i in 1:length(dates)){
    ...
    if(vtable){
        ...
        tables[[i]] <- st(data[date == date.now, ..sumvars], out = "kable" )
        
        st(data[date == date.now, ..sumvars], out = "latex",
           file = paste( tables.folder, "/sumtable", as.character(i) , ".tex", sep = "") )
            
        #summarytools::descr(data[date == date.now, ..sumvars], stats = stats , transpose = TRUE,
        #style = "rmarkdown", round.digits = 1) %>%  tb()
        
    } else {
        
        f <- as.formula( paste( paste(sumvars, collapse = "+") , "~", paste(stats, collapse = "+")   )     )
        
        datasummary( f  , data = data[date == date.now, ..sumvars] ,
                     output = paste( tables.folder, "/sumtable", as.character(i) , ".tex", sep = "") ) 
    }
...
new attempt using modelsummary package. The problem is that for some reason the command does not calculate some statistics.

```{r summary stats levels 2, echo=FALSE, warning=FALSE}
